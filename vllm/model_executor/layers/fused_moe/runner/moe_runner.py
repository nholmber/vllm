    ...
    def _forward_impl(
        ...
        if self.gate is not None:
            if self._fse_fuse_gate:
                ...
                # Route through the platform GEMM dispatcher rather than
                # F.linear: on ROCm the un-fused branch below reaches the
                # skinny-GEMM kernels via ReplicatedLinear, and calling
                # F.linear here would silently drop the fused gate onto
                # generic rocBLAS instead.
