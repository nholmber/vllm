...
class Qwen3NextSparseMoeBlock(nn.Module):
    ...
    def forward(self, hidden_states: torch.Tensor) -> torch.Tensor:
        ...
        if self.is_fusion_moe_shared_experts_enabled:
            # FSE path: the fused MoE kernel already computed the shared
            # expert output combined with routed experts.
            # Result is (shared_out, fused_out) tuple — take fused_out.
...
class Qwen3NextModel(nn.Module, EagleModelMixin):
    ...
    def load_weights(self, weights: Iterable[tuple[str, torch.Tensor]]) -> set[str]:
        ...
        # When FSE is enabled, remap shared_expert checkpoint weights
        # to the fused expert slot (expert index = num_routed_experts).
